...
export function writeGluingModules(tsccSpec: ITsccSpecWithTS, tsickleHost: TsickleHost, writeFileImpl: (path: string, content: string) => void) {
	const moduleNames = tsccSpec.getExternalModuleNames();
	const toGlobalName = tsccSpec.getExternalModuleNamesToGlobalsMap();
	for (let moduleName of moduleNames) {
		const content = `goog.module(${tsickleHost.pathToModuleName('', moduleName)})\n` +
			`/** Generated by TSCC */\n` +
			`exports = ${toGlobalName[moduleName]};`;
		let typeRefFile = tsccSpec.resolveExternalModuleTypeReference(moduleName) || moduleName;
		writeFileImpl(typeRefFile.replace(/(?:\.d)?\.ts$/, '.js'), content)
	}
}
